...
- **轻量级 RAG 记忆检索 (Local File-System RAG)**：摒弃了无限膨胀的单一 `ctx.md`，转为**按月时间分片** (`ctx-YYYY-MM.md`)。AI 可调用 `ritsu_retrieve_memory` 工具，利用本地文件全文检索技术（Keyword/Semantic），横跨数年精准调取历史架构决策与 Bug 诊断。
...
| `ritsu/ctx-{YYYY-MM}.md` | 所有技能 | 任务执行日志时间分片，会话恢复的情景记忆 |
...
### ctx-protocol.md
情景记忆按月分片与持久化协议。每个技能在启动和完成时追加一条记录到 `ritsu/ctx-{YYYY-MM}.md`，格式为：
```
...
### mcp-tools.md
包含 7 个带有强防幻觉拦截能力的 MCP Tool Schema 声明（含 RAG 检索）。把 AI 的工具调用从"解读自然语言指令"升级为"按 Schema 确定性调用"，并严格限制并发原子锁与大文件屏蔽。

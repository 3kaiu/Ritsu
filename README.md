...
## Golden Path（第一次使用：从需求到 Review Stamp）

> 建议把这段当作“最短上手路径”，先跑通闭环，再看后面的协议细节。

```text
# 1) 初始化（生成 AGENTS.md / .gitignore 注入 / 领域识别）
/r-init

# 2) 直接跑端到端流水线（think → dev → review）
/r-pipe standard "实现：用户登录态持久化 + 自动续期"

# 3) 通过审查后，会生成：
.ritsu/handoff-*.md
.ritsu/review-stamp-*.md
.ritsu/ctx-YYYY-MM.jsonl
```

如果你想看这次闭环的“人类可读执行记录”（彩色高亮）：

```bash
ritsu cat <cid-YYYYMMDD-N>
```

（GIF/录屏建议：建议录一段从 `/r-pipe standard ...` 到 Review PASS 的 30s 演示，放到 `assets/golden-path.gif` 并在此处引用。）
...
```text
.ritsu/
  handoff-*.md           # think 产物：契约 + 实施清单
  review-stamp-*.md      # review 产物：结论 + hard stop + 红蓝对抗发现
  diagnosis-*.md         # hunt 产物：证据链 + 根因
  optimize-report-*.md   # optimize 产物：优化报告
  ctx-YYYY-MM.jsonl      # 全技能事件流（started/done/failed/artifact_written）
```

...
│   ├── skill-common-steps.md   # 技能公共步骤模板（结构化输出协议/领域解析/L0-L2跳步映射/ctx写入/关联流转）
...
技能公共步骤模板，包含：Step 0 Pre-flight + 执行模式选择（fast/standard）、Step 1 领域解析（三优先级协议）、Step 2 ctx 写入（4 种核心事件模板 + 失败恢复）、Step 3 关联流转（引用 state-machine.yaml）。所有技能必须引用此文件，**禁止各自实现领域判断或熔断规则**。

### artifact-schema.yaml

定义 5 种产物的强制格式（Schema 0-4，统一 Markdown）。`ritsu_write_artifact` 工具在写入时会校验格式合规性，发现占位符自动拒绝写入（Schema 0 AGENTS.md 豁免）。Handoff 产物支持 append-only Changelog 追踪契约变更。
...
| 版本   | 核心变更                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v3.7.0 | **去冗余 + 交付闭环 + 复杂度可执行化**：移除 domain-resolver.md（内容已内联至 skill-common-steps.md，单一事实来源）；移除 state-machine.yaml 中 transitions/ui_hint 死代码（无 UI 消费者）；移除 ritsu_validate 工具（emit_event 自动校验，无 skill 调用独立校验）；移除 artifact-schema.yaml 中 dual_format_protocol 遗物 section；简化 correlation_id 机制（ritsu_emit_event 自动生成+继承，无需手动指定）；简化月度摘要为归档提示（100 条提示归档）；统一 anti-patterns 与 HC 关系（HC 标注 =AP-N 引用，不重写规则）；skill-common-steps Step 3 熔断规则改为引用 state-machine.yaml（消除重复定义）；修复 context-loader Phase 2 空操作（补充 ritsu_read_ctx 恢复检查）；补全 L0/L1/L2 各技能跳步映射表（复杂度分级从声明变为可执行）；新增 deploy 技能（预发布检查→部署→冒烟→回滚方案）；新增 test 技能（策略→用例→执行→覆盖率）；review PASS 后新增 deploy/test 引导选项。 |
| v3.6.0 | **事件精简 + 业务工具回归 + 上下文增强**：事件 Schema 从 10 种精简为 4 种核心类型（started/done/failed/artifact_written），移除 approval/step_done/transition/circuit_breaker 等事件类型，审批和熔断告警改为 AI 自然语言输出；恢复 3 个高频业务工具（ritsu_get_changed_files/ritsu_get_diff/ritsu_run_quality_gates）为结构化 MCP 工具，替代 AI 自行组合 ritsu_exec 的脆弱模式；ritsu_read_ctx 增强 recovery_context（可操作恢复上下文）+ reality_check（产物文件存在性对账）+ circuit_breaker_status（连续失败计数与重定向建议）；移除 html 双格式输出，所有产物统一 Markdown；新增任务复杂度分级（L0 微调/L1 常规/L2 标准），L0 快速通道跳过 think/review；8 个 SKILL.md 适配新工具与精简事件流。                                                                                                                                                                             |
| v3.5.1 | **架构加固 (Architecture Hardening)**：exec 安全边界从黑名单改为白名单+残余黑名单双层机制；execSync 改为 spawn+Promise 非阻塞执行；WASM 索引/加载加 async mutex 防并发竞态；readLastIncomplete/getNextSeq 改为全量扫描消除窗口截断丢失；JS/WASM 状态漂移修正（失败时从文件重算行数）；提取 shared.ts 消除 3 处 getSharedDir 重复 + validTypes/prefixMap 双重重复 + 安全边界常量重复；删除 5 处死代码；Schema 对齐（artifact/progress 显式 null、started 必填 step）；CI 补 vitest + cargo test + version:check；Rust 三模块补 21 个单元测试；业务逻辑工具删除（AI 自行组合 ritsu_exec 原语）。                                                                                                                                                                                                                                                                                  |
| v3.5.0 | **MCP Server 运行时 (Runtime Implementation)**：新增 runtime/ 目录（TypeScript MCP Server），将 mcp-tools.yaml 协议声明编译为可执行 MCP 工具服务；6 个 SDK 工具完整运行时实现（ritsu_emit_event/ritsu_read_ctx/ritsu_write_artifact/ritsu_list_artifacts/ritsu_exec/ritsu_validate）；ctx-event-schema.json + ajv 运行时校验；correlation_id 生成/继承行为闭环；route 增加 transition_event 写入；stdio transport 支持 IDE 集成（Cursor/Windsurf/Claude Desktop）。                                                                                                                                                                                                                                                                                                                                                                                                             |
| v3.4.0 | **协议标准化 (Protocol Standardization)**：ctx 事件从 3 态(started/done/failed)扩展为全生命周期事件流(step_done/step_failed/approval_required/artifact_written/circuit_breaker)；新增 correlation_id 任务链路关联（route 生成，同链路继承）；9 个 MCP 工具增加 output_schema（含新增 ritsu_emit_event 专用事件写入工具）；状态机增加 transitions+ui_hint（UI 可做状态动画）；新增审批协议(approval_required/granted/denied)；新增 violation/transition 事件格式；8 个 SKILL.md 增加 total_steps 声明；skill-common-steps.md 增加 step 级 ctx 写入模板+correlation_id 继承规则；ritsu_write_artifact 增加 artifact_meta 参数；新增 ctx-event-schema.json 机器可读 Schema。所有改动向后兼容，旧 LLM 忽略新字段照常工作。                                                                                                                                                          |

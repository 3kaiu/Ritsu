...
│   └── mcp-tools.yaml          # MCP Tool Schema 声明（9 个工具 + 结构化调用模板 + output_schema）
│
...
│   │   └── handlers/           # 9 个工具的运行时 handler (6 SDK + 3 业务)
│   ├── core/                    # Rust WASM 核心模块
│   │   └── src/
...
包含 9 个工具的 MCP Tool Schema 声明（含结构化调用模板）。YAML 源文件可编译为 JSON Schema 供 MCP 运行时注册。每个工具附带 `call_template` 和 `example`，LLM 按模板确定性构造调用参数，禁止自由推断。

> **工具定位**：6 个 SDK 原语（`emit_event`/`read_ctx`/`write_artifact`/`list_artifacts`/`exec`/`validate`）+ 3 个高频业务封装（`get_changed_files`/`get_diff`/`run_quality_gates`）。业务封装工具将 AI 频繁组合的 `ritsu_exec` 调用固化为结构化输出，减少指令歧义和执行失败率。

...
│   └── mcp-tools.yaml          # MCP Tool Schema 声明（6 个 SDK 工具 + 结构化调用模板 + output_schema）
│
...
│   │   └── handlers/           # 6 个 SDK 工具的运行时 handler
│   ├── core/                    # Rust WASM 核心模块
│   │   └── src/
...
包含 6 个 SDK 工具的 MCP Tool Schema 声明（含结构化调用模板）。YAML 源文件可编译为 JSON Schema 供 MCP 运行时注册。每个工具附带 `call_template` 和 `example`，LLM 按模板确定性构造调用参数，禁止自由推断。

> **SDK 工具定位**：Ritsu MCP Server 是纯 SDK（结构化 I/O 原语），不做业务决策。业务逻辑工具（`ritsu_get_changed_files`、`ritsu_get_diff`、`ritsu_grep_identifier`、`ritsu_run_quality_gates`、`ritsu_retrieve_memory`）已删除，AI 自行调用 `ritsu_exec` 等原语组合实现。

...
| 会话重置丢上下文    | 新对话不知道做到哪一步了         | `ctx-{YYYY-MM}.md` 记忆分片，支持 RAG 语义回溯   |
...
│   ├── artifact-schema.yaml    # 产物格式契约（Schema 0-5 + 双格式协议）
│   ├── state-machine.yaml      # 全局流转路径与标准引导话术
│   ├── ctx-protocol.md         # 情景记忆持久化协议（.ritsu/ctx.jsonl）
│   └── mcp-tools.yaml          # MCP Tool Schema 声明（8 个工具）
│
...
定义 7 种产物的强制格式（Schema 0-5 + 双格式协议）。`ritsu_write_artifact` 工具在写入时会校验格式合规性，发现占位符自动拒绝写入。面向人类的产物（Diagnosis / Review Stamp）同时输出 md 和 html 双文件。
...
`/r-route` 启动时自动读取，识别 Git 现实对账后询问是否恢复。

### mcp-tools.yaml

包含 8 个带有强防幻觉拦截能力的 MCP Tool Schema 声明（含 RAG 检索）。YAML 源文件可编译为 JSON Schema 供 MCP 运行时注册。把 AI 的工具调用从"解读自然语言指令"升级为"按 Schema 确定性调用"，并严格限制并发原子锁与大文件屏蔽。

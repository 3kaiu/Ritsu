...
- intake-ticket
- delivery-plan（若已存在，优先对齐其目标范围与验证计划）
- 最近的 assurance-report（优先）
- release-advice（若本次为发布姿态回流）
- 仅当主链路产物不足以解释当前问题时，再补充 handoff / diagnosis / review-stamp 等过程证据
- 当前需求文本

若需要检索 `.ritsu/` 历史记录，默认策略为：

1. 优先调用 `ritsu_semantic_search`，使用 `layers=["primary"]`
2. 若主链路产物信息不足，再扩展为 `layers=["evidence"]`
3. 只有下游仍依赖 legacy 镜像时，才额外查看 `layers=["compatibility"]`
...
若 `delivery-plan` 已经足够承载本次实施计划，`think` 可以只补充设计结论，不强制重复写 handoff。

只有在以下情况之一成立时，才应调用 **`ritsu_write_artifact`**（type=handoff）写入细粒度设计产物：

- 需要补充接口 / 数据 / 组件级契约
- 需要给 `dev` 提供可直接执行的实施清单
- `critical` 任务需要把复杂边界落成可审计设计证据
...
- `delivery-plan` 负责主链路实施目标、步骤、验证计划、回滚说明

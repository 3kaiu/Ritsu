...
📋 优化分析清单
├── 死代码：{未使用的变量/函数/导入/类型}
├── 冗余逻辑：{可合并的条件分支/重复计算/可内联的单次调用函数}
├── 算法热点：{O(n²)→O(n) 机会 / 不必要的全量遍历 / 可 memo 化的计算}
├── 工具库替换：{手写逻辑 → 领域推荐工具函数（见 domains/*.yaml optimize_tool_preferences）}
├── 语义化标签：{div/span → header/main/section/nav/article/aside/time}
├── 注注清理：{无用注释删除 | 核心 TSDoc/JSDoc 保留}
├── 样式精简：{内联/冗余 CSS → TailwindCSS 等价类 | 重复样式合并}
├── 平台优化：{WebView 专属优化（见 domains/*.yaml platform_optimizations）}
└── 请求优化：{接口请求模式 → 领域推荐请求工具（见 domains/*.yaml optimize_tool_preferences）}
```

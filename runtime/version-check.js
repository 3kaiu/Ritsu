...
export function checkVersions(targetProjectRoot, WRITE = false) {
  ...
  if (!expected) {
    throw new Error("package.json: ritsu_protocol_version or version missing");
  }
  ...
  for (const t of targets) {
    const absPath = path.join(targetProjectRoot, t.file);
  ...
  return { expected, mismatches, writes };
}
...
if (isEntryPoint) {
  ...
  try {
    const { expected, mismatches, writes } = checkVersions(projectRoot, WRITE);
